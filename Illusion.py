...
async def start_illusion():
    ...
    if smex:
        ...
        try:
            ...
            await idk(functions.channels.JoinChannelRequest(channel="@OP_SQUAD"))
    ...
    if smexxx:
        ...
        try:
            ...
            await  wdk.start()
            await wdk(functions.channels.JoinChannelRequest(channel="@OP_SQUAD"))
    ...
    if smexxxxx:
        ...
        try:
            ...
            await sdk.start()
            await sdk(functions.channels.JoinChannelRequest(channel="@OP_SQUAD"))
    ...
    if sixth:
        ...
        try:
            ...
            await adk.start()
            await adk(functions.channels.JoinChannelRequest(channel="@OP_SQUAD"))
    ...
    if seven:
        ...
        try:
            ...
            await bdk.start()
            await bdk(functions.channels.JoinChannelRequest(channel="@OP_SQUAD"))
    ...
    if eight:
        ...
        try:
            ...
            await cdk.start()
            await cdk(functions.channels.JoinChannelRequest(channel="@OP_SQUAD"))
    ...
    if ninth:
        ...
        try:
            ...
            await ddk.start()
            await ddk(functions.channels.JoinChannelRequest(channel="@OP_SQUAD"))
    ...
    if tenth:
        ...
        try:
            ...
            await edk.start()
            await edk(functions.channels.JoinChannelRequest(channel="@OP_SQUAD"))
...
loop.run_until_complete(start_illusion())       
...
async def _(e):
    ...
    if e.sender_id in SMEX_USERS:
        illusion = ("".join(e.text.split(maxsplit=1)[1:])).split(" ", 1)     
        if len(e.text) > 5:
            bio = str(illusion[0])
            text = "𝕔𝕙𝕒𝕞𝕘𝕚𝕟𝕘 𝕞𝕒𝕙 𝕓𝕚𝕠...."
            event = await e.reply(text, parse_mode=None, link_preview=None )
            try:
                await e.client(functions.account.UpdateProfileRequest(about=bio))
                await event.edit("𝕊𝕦𝕔𝕖𝕤𝕤𝕗𝕦𝕝𝕝𝕪 ℂ𝕙𝕒𝕟𝕘𝕖𝕕 𝔹𝕚𝕠")
            except Exception as e:
...
async def _(e):
    ...
    if e.sender_id in SMEX_USERS:
        ...
        if len(e.text) > 6:
            bc = illusion[0]
            text = "𝕁𝕠𝕚𝕟𝕚𝕟𝕘 𝕤𝕒𝕣 𝕡𝕝𝕠𝕩 𝕨𝕒𝕚𝕥...."
            event = await e.reply(text, parse_mode=None, link_preview=None )
            try:
                await e.client(functions.channels.JoinChannelRequest(channel=bc))
                await event.edit("𝕐𝕖𝕒𝕙❕ 𝕁𝕠𝕚𝕟𝕖𝕕 𝕤𝕦𝕔𝕔𝕖𝕤𝕗𝕦𝕝𝕝𝕪")
            except Exception as e:
                await event.edit(str(e))   
        else:
            await e.reply(usage, parse_mode=None, link_preview=None )
...
async def _(e):
    ...
    if e.sender_id in SMEX_USERS:
        ...
        if len(e.text) > 7:
            bc = illusion[0]
            bc = int(bc)
            text = "𝕦𝕗𝕗❕ 𝕝𝕖𝕒𝕧𝕚𝕟𝕘 𝕤𝕒𝕣...."
            event = await e.reply(text, parse_mode=None, link_preview=None )
            try:
                await event.client(LeaveChannelRequest(bc))
                await event.edit("𝕠𝕞𝕜 , 𝕃𝕖𝕗𝕥 𝕊𝕦𝕔𝕔𝕖𝕤𝕗𝕦𝕝𝕝𝕪")
            except Exception as e:
                await event.edit(str(e))   
        else:
            await e.reply(usage, parse_mode=None, link_preview=None )
...
async def _(e):
    usage = "𝗠𝗼𝗱𝘂𝗹𝗲 𝗡𝗮𝗺𝗲 = 𝐒𝐭𝐢𝐜𝐤𝐞𝐫 𝐒𝐩𝐚𝐦\n\nCommand:\n\n.sspam\n\n.sspam <reply to a Sticker>."
...
async def spam(e):
    ...
    if e.sender_id in SMEX_USERS:
        ...
        if len(illusion) == 2:
            message = str(illusion[1])
            counter = int(illusion[0])
            if counter > 100:
...
async def spam(e):
    ...
    if e.sender_id in SMEX_USERS:
        if e.text[0].isalpha() and e.text[0] in ("/", "#", "@", "!"):
            return await e.reply(usage, parse_mode=None, link_preview=None )
        smex = await e.get_reply_message()
        illusion = "".join(e.text.split(maxsplit=1)[1:]).split(" ", 2)
        sexy = illusion[1:]
        if len(sexy) == 2:
            message = str(sexy[1])
            counter = int(sexy[0])
            sleeptime = float(illusion[0])
            for _ in range(counter):
...
print(text)
print("")
print("Yeah boi! Illusion Spam Bot Started Sucessfully.")
